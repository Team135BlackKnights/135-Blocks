...
public class ElevatorC extends Command {
	...
	public void initialize() {
		StateSpaceConstants.Controls.go0ftButton.onTrue(new InstantCommand(() -> {
			elevatorPos = Units.feetToMeters(0);
			elevatorS.moveElevator(elevatorS.createState(elevatorPos));
		}));
		StateSpaceConstants.Controls.go2ftButton.onTrue(new InstantCommand(() -> {
	...
	public void execute() {
		double elevatorSpeed = 0,
				desSpeed = -RobotContainer.manipController.getLeftY();
		...
		if (Math.abs(desSpeed) > StateSpaceConstants.Controls.kArmDeadband) {
			if (elevatorPos >= StateSpaceConstants.Elevator.maxPosition
					&& desSpeed > 0) {
		...
		if (elevatorSpeed == 0) {
			...
			elevatorS
					.moveElevator(elevatorS.createState(elevatorPos, elevatorSpeed));
		}
	}
